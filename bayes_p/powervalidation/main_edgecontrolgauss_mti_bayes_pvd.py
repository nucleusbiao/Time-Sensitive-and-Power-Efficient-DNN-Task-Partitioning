#功耗模型在线验证, edge 上运行。
#随机给出分割点
#用户设备运行：main_clientcontrolbaues_vp.py
#功耗收集设备运行：powerofpp_vp.py
...
def edge(dnn,netnumer):
    ...
    time.sleep(2)
    print(dnn," edge closed now.")
#HOST,port,totalnumber,partitioncountdict[dnn],username
...
def black_box_function(next_point_to_probe,numberlist,powermodel):#该设备上任务根据分割点做出决策，等待运行一段时间后,返回能耗
    ...
    powerlistpredicted=powermodel.predict(cpulist_)
    #缩放，0~1
    result = [(30.0 / power - 2)/8.0 for power in powerlistpredicted]
    # powerresult = [-1.0* power for power in powerlist]
    print('balck box pp is :',next_point_to_probe,"powerlistreal: ",powerlistreal,"powerlistpredicted: ",powerlistpredicted," cpulist: ",cpulist)

    return powerlistreal,powerlistpredicted,cpulist
...
def overallcontrol(username): #每个设备上有一个 username，设备名称
    ...
    while True:
        ...
        bayesf=str(parent_dir)+"/latency/validlog"+username+str(taskchangecount[username])+".txt"
        with open(bayesf,"w") as f:
            for i in range(len(numberlist)):
                f.write(f'partition_point{str(numberlist[i])}         ')
            for i in range(len(numberlist)):
                f.write(f'cpu{str(numberlist[i])}         powerreal{str(numberlist[i])}        powerpredicted{str(numberlist[i])}             ')    
            f.write("\n")
        ...
        while taskchangeflag[username]==False:
            ...
            #2.等待一段时间，获取设备能耗。
            powerlistreal,powerlistpredicted,cpulist = black_box_function(next_point_to_probe,numberlist,powermodel)

            with open(bayesf,"a") as f:
                for number in numberlist:
                    f.write(str(round(next_point_to_probe[number])))
                    f.write("    ")
                for i in range(len(powerlistreal)):
                    f.write(str(cpulist[i]))
                    f.write("    ")
                    f.write(str(powerlistreal[i]))
                    f.write("    ")
                    f.write(str(powerlistpredicted[i]))
                    f.write("    ")
                f.write("\n")
...
timecounter=0
bandwidthlist=[55,55,55,8,8,8,15,15,15,5,5,5]
