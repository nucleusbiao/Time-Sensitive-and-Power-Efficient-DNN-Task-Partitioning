...
def black_box_function(next_point_to_probe,numberlist,powermodel,username):#该设备上任务根据分割点做出决策，等待运行一段时间后,返回能耗
    ...
    #print('balck box pp is :',next_point_to_probe,"powerlist: ",powerlist,"power result ",result)
...
def get_constrainprobi(constraintvalue,numberlist,pbounds_d,constraint_ulb):   #constraintvalue #[(arrarmu1,arrarysov1),]
    ...
    result = []
    j=0
    for predicted in constraintvalue:
        ...
        j+=1
        result.append((p_upper-p_lower+0.01))

    if len(result)!=len(numberlist):  #！！！！！！！！！！！！ 长度不够！！！！！！！！！！！！！
        result.append(np.ones((32,)))
        print("\033[91m生成了一组constraintvalue.\033[0m")
    #都是1好了。
    if len(result)!=len(numberlist):  #！！！！！！！！！！！！ 长度不够！！！！！！！！！！！！！
        result.append(np.ones((32,)))
        print("\033[91m生成了一组constraintvalue.\033[0m")
...
def getfitness(powerlist,constrainprobi,next_point_to_probe,numberlist):

    partitionlist=[]
    fitnessl=[]

    for i in range(len(numberlist)):
        pp=next_point_to_probe[numberlist[i]]
        fitness=-constrainprobi[i][pp]*(100.0/powerlist[0])-10
        if np.isnan(fitness) or fitness<0:
            fitness=1
        fitnessl.append(fitness)
    return fitnessl    #[net1_p_fitness,net2_p_fitness]
...
def overallcontrol(username:str): #每个设备上有一个 username，设备名称
    ...
    while True:
        ...
        #限制条件 
        #都设置为1.5s
        tasksnumber_device=len(numberlist)
        ...
        #用于存放遗传算法提供的种群个体
        GAplist=[]
        fitnesslist=[[] for i in range(tasksnumber_device)]  # #用于存放个体的得分
        #遗传算法种群栈，都执行后进行交叉融合。
        GAstack=[]
        ...
        gasize=8   #个体数
        galength=20 #基因长度
        gaoptimizer=GA(gasize,galength,tasknamelist)
        gaDNA=gaoptimizer.translateDNA()
        print("gaDNA: ", gaDNA)
        for gA in gaDNA:
            GAplist.append(gA.tolist()[:])   #[[ga_task1],[ga_task2],..]
        GAstack=copy.deepcopy(GAplist)
    
        #记录
        bayesf=str(parent_dir)+"/latency/galog"+username+str(taskchangecount[username])+".txt"
        with open(bayesf,"w") as f:
            ...
            f.write("     poweraver")
            f.write("    suggest_time   blackbox_time    update_time\n")
        ...
        for i in range(4):
            ...
            #print("next_point_to_probe is ",next_point_to_probe)
        ...
        while taskchangeflag[username]==False:
            ...
            if len(GAstack[0])==0:  #遗传算法，做更新
                #GAupdateflag=True
                gaoptimizer.crossover_and_mutation()
                #print("fitnesslist: ",fitnesslist)
                gaoptimizer.select(np.array(fitnesslist))
                gaDNA=gaoptimizer.translateDNA()
                #print("gaDNA: ", gaDNA)
                GAplist=[]
                fitnesslist=[[] for i in range(tasksnumber_device)]
                for gA in gaDNA:
                    GAplist.append(gA.tolist()[:])   #[[ga_task1],[ga_task2],..]
                GAstack=copy.deepcopy(GAplist)
                #print("GAstack ", GAstack)
                #print("GAplist ", GAplist)
            sugendtime=time.time()

            for i in range(tasksnumber_device):
                next_point_to_probe[numberlist[i]]=GAstack[i].pop(0)
            #print("next_point_to_probe is ",next_point_to_probe)
            ...
            constraintvalue=constraint_function(next_point_to_probe,numberlist)
            #print("constraintvalue: ",constraintvalue)
            #3.计算概率
            constrainprobi=get_constrainprobi(constraintvalue,numberlist,pbounds_d,constraint_ulb)  #[[parrary1],[parrar2],]
            #print("constrainprobi: ",constrainprobi)
            #print("constraintvalue length ",len(constraintvalue)," ",constraintvalue[0][0].shape)

            #4.计算fitness
            fitness=getfitness(powerlist,constrainprobi,next_point_to_probe,numberlist)
            for i in range(tasksnumber_device):
                fitnesslist[i].append(fitness[i])
            upendtime=time.time()
