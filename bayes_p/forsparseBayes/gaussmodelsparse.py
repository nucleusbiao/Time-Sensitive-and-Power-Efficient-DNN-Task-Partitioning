...
print("guass parent_parent_dir",parent_parent_dir)
...
class Gaussmodel():    
    ...
    def predictall(self):
        ...
        train_x=[]
        length=min(len(self.latencypool_trans),len(self.latencypool_client),len(self.latencypool_edge))
        #随机采样20个点。
        batch_size=20
        if batch_size > length:
            batch_size = length
        indexes = np.random.choice(np.arange(length), size=batch_size, replace=False)

        for i in range(3):
            train_y=[]
            pplist=[]
            if i==0:
                train_y=[self.latencypool_client[j] for j in indexes]
                pplist=[self.latencypool_clientpp[j] for j in indexes]
            elif i==1:
                train_y=[self.latencypool_trans[j] for j in indexes]
                pplist=[self.latencypool_transpp[j] for j in indexes]
            else:
                train_y=[self.latencypool_edge[j] for j in indexes]
                pplist=[self.latencypool_edgepp[j] for j in indexes]
