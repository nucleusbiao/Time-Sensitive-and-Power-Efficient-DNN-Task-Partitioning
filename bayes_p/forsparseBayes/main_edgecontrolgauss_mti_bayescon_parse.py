...
import time
import threading
from gaussmodelsparse import Gaussmodel
import math
import json
import numpy as np
...
def guassthread(dnnnumber,username,dnn):
    ...
    for epoch in range(30000):
        if readyflag ==False:  #判断数据是否满足
            if len(Gausspro.latencypool_clientpp) >=mindatanum and len(Gausspro.latencypool_transpp) >=mindatanum and len(Gausspro.latencypool_edgepp) >=mindatanum:
                readyflag=True 
...
def black_box_function(next_point_to_probe,numberlist,powermodel,username):#该设备上任务根据分割点做出决策，等待运行一段时间后,返回能耗
    ...
    dnnsum=len(next_point_to_probe)
    ...
    return powerresult,powerlist,cpucountlist
...
def overallcontrol(username:str): #每个设备上有一个 username，设备名称
    ...
    while True:
        ...
        #记录
        bayesf=str(parent_dir)+"/latency/spasrebayeslog"+username+str(taskchangecount[username])+".txt"
        with open(bayesf,"w") as f:
            ...
            f.write("poweraver     suggest_time   blackbox_time    update_time   iteratetime") 
            f.write("\n")
        ...
        for i in range(5):
            ...
            if taskchangeflag[username]==False:
                ...
                interatetime=time.time()-overstarttime
                if taskchangeflag[username]==True:
                    break
