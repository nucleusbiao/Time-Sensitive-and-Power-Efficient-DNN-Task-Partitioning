...
def black_box_function(next_point_to_probe,numberlist,powermodel):#该设备上任务根据分割点做出决策，等待运行一段时间后,返回能耗
    global ddpartitionlist
    global cputotalsum
    global cputotallist
...
def overallcontrol(username:str,mode): #每个设备上有一个 username，设备名称
    ...
    while True:
        ...
        #记录
        powerf=str(parent_dir)+"/latency/powerlog"+username+str(taskchangecount[username])+".txt"
        ...
        while taskchangeflag[username]==False:
            ...
            #2.等待一段时间，获取设备能耗。
            powerresult,powerlist = black_box_function(next_point_to_probe,numberlist,powermodel)
