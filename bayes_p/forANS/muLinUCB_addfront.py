...
class muLinUCB():
    def __init__(self, mu, layerInfof, layerInfoe):
       ...
       # self.frontDelay = frontDelay
        ...
        self.alphaf = 0.5*(self.C_theta + np.sqrt(np.log((1 + self.frameNum * self.C_x * self.C_x)/self.delta) * self.thetaContextDim_f)*self.C_noise)/(1 - self.l_key)
        print(self.alphaf.item())
        print(self.alphaf.item())
        print(self.alphaf.item())
        #self.alpha =0.85

        self.alphae = 0.5*(self.C_theta + np.sqrt(np.log((1 + self.frameNum * self.C_x * self.C_x)/self.delta) * self.thetaContextDim_e)*self.C_noise)/(1 - self.l_key)
