...
if args.devicename=='k':
    dnnlist=["yolo","resnet","alexnet","mobilenetv2"]
    sarttimelist=[0,4,12,16]
    timelength=[10,12,10,10]
    portlist=[9050,9051,9052,9053,9054,9055,9056]
    # dnnlist=["vgg","yolo"]
    # sarttimelist=[0,0,12,16]
    # timelength=[24,24,10,10]
    # portlist=[9050,9051,9052,9053,9054,9055,9056]
elif args.devicename=='u':
    dnnlist=["vgg","alexnet"]
    sarttimelist=[0,10,16,26]
    timelength=[10,16,10,6]
    portlist=[9070,9071,9066,9067]
    # dnnlist=["yolo","resnet"]
    # sarttimelist=[0,0,16,26]
    # timelength=[24,24,10,6]
    # portlist=[9064,9065,9066,9067]
elif args.devicename=='tx2':
    dnnlist=["vgg"]
    sarttimelist=[0,0,6,9,12]
    timelength=[26,24,14,11,8]
    # dnnlist=["resnet",'alexnet']
    # sarttimelist=[0,0,6,9,12]
    # timelength=[24,24,14,11,8]
    portlist=[9030,9031,9032,9033,9034,9035]
elif args.devicename=='ly':   
    dnnlist=["alexnet","resnet"]
    sarttimelist=[0,10,10,28]
    timelength=[12,16,6,4]
    portlist=[9080,9081,9082]
    # dnnlist=["mobilenetv2","alexnet","vgg"]
    # sarttimelist=[0,0,0,28]
    # timelength=[24,24,24,4]
    # portlist=[9080,9081,9082]
else:
        ...
        if timenow-timestart>=(timecounter+1)*60:
            ...
            for i in range(len(dnnlist)):
                if(sarttimelist[i]==timecounter):
                    ...
                    startargs='python3 ' +str(this_dir)+'/client_ans.py --host '+args.host+ ' --port='+str(portlist[i])+' --dnn_model='+str(dnnlist[i])+ ' --breaktime='+str(timelength[i])+\
                                ' --netnumber='+str(i)
                    #print(startargs)
