...
class AcquisitionFunction(abc.ABC):
    ...
    def suggest(
        ...
        acq = self._get_acq(gp=gp, constraint=target_space.constraint,constraint_w=target_space._constraint_w)
        return self._acq_min(acq, target_space.bounds, n_random=n_random, n_l_bfgs_b=n_l_bfgs_b)

    def _get_acq(
        self,constraint_w, gp: GaussianProcessRegressor, constraint: ConstraintModel | None = None
        ...
        if constraint is not None:
            ...
            def acq(x: NDArray[Float]) -> NDArray[Float]:
                ...
                with warnings.catch_warnings():
                    ...
                    mean, std = gp.predict(x, return_std=True)  #x shape(10000,1)
                    p_constraints = constraint.predict(x)
                return -1 * self.base_acq(mean, std) * p_constraints
            
        elif constraint_w is not None:
            ...
            def acq(x: NDArray[Float]) -> NDArray[Float]:
                ...
                with warnings.catch_warnings():
                    ...
                    std: NDArray[Float]
                    p_constraints: NDArray[Float]
                    mean, std = gp.predict(x, return_std=True)  #x shape(10000,n) nΪ�Ա�������
                    p_constraints = constraint_w.predict(x)
                    base_acq=self.base_acq(mean, std)
                    result=-1 * self.base_acq(mean, std) * p_constraints
                    # file="latency/"+str(self.countfile)+".txt"
                    # if x.shape[0]>20 and x.shape[1]>1:
                    #     with open(file,"w") as f:
                    #         f.write(f'x[0]  x[1]   mean   std    p_constraints   base_acq   result\n'    )
                    #         for i in range(x.shape[0]):
                    #             f.write(f'{str(x[i][0]):<30}{str(x[i][1]):<30}{str(mean[i]):<30}{str(std[i]):<30}{str(p_constraints[i]):<30}{str(base_acq[i]):<30}{str(result[i]):<30}')
                    #             f.write("\n")
                    #     self.countfile+=1
                    # elif x.shape[0]>20:
                    #     with open(file,"w") as f:
                    #         f.write(f'x[0]   mean   std    p_constraints   base_acq   result\n'    )
                    #         for i in range(x.shape[0]):
                    #             f.write(f'{str(x[i][0]):<30}{str(mean[i]):<30}{str(std[i]):<30}{str(p_constraints[i]):<30}{str(base_acq[i]):<30}{str(result[i]):<30}')
                    #             f.write("\n")       
                    #     self.countfile+=1
                    #     plt.ion()
                    #     ax = plt.axes()
                    #     ax.scatter(x[:,0], base_acq)
                    #     #plt.ioff()
                    #     plt.pause(0.9)#暂停time秒
                    #     plt.show()
                    #     #print(TargetSpace.ymax)

                    # print("x",x[0:20])
                    # print("mean: ",mean[0:20])
                    # print("std: ",std[0:20])
                    # print("p_constraints: ",p_constraints[0:20])
                    # print("base_acq ",base_acq[0:20])
                    # print("result: ",result)
...
class ProbabilityOfImprovement(AcquisitionFunction):
    ...
    def decay_exploration(self) -> None:
        ...
        """
        if self.exploration_decay is not None and (
            self.exploration_decay_delay is None or self.exploration_decay_delay <= self.i and self.xi>-0.5
        ):
