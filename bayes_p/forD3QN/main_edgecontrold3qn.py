...
def guassthread(dnnnumber,username,dnn):
    ...
    pprf=str(parent_dir)+"/latency/d3qnlatency"+str(dnn)+str(dnnnumber)+".txt"
...
def black_box_function(next_point_to_probe,numberlist,powermodel,username):#该设备上任务根据分割点做出决策，等待运行一段时间后,返回能耗
    ...
    dnnssum=len(numberlist)
    for i in range(dnnssum):
        lockc.acquire()
    ...
    #缩放，0~1
    powerresult = [(30.0 / power - 2)/8.0 for power in powerlist]

    return powerresult,powerlist,latencylist,cpucountlist
...
def overallcontrol(username:str): #每个设备上有一个 username，设备名称
    ...
    while True:
        ...
        # 模型构建
        # ----------------------------------------- #
        target_network = Dueling_DQN(n_states, n_actions,dnn_sum,ppmaxlist).to(device)
        network = Dueling_DQN(n_states, n_actions,dnn_sum,ppmaxlist).to(device)
        optimizer = torch.optim.Adam(network.parameters(), lr=net_lr)
        StepLR = torch.optim.lr_scheduler.StepLR(optimizer, step_size=80, gamma=0.45)
        ...
        #记录
        ppof="latency/d3qnlog"+username+str(taskchangecount[username])+".txt"
        with open(ppof,"w") as f:
            ...
            f.write("poweraver     suggest_time   blackbox_time    update_time   loss") 
            f.write("\n")

        next_point_to_probe=[]    # list ,对应numberlist,action
        ...
        while taskchangeflag[username]==False:
            ...
            if (cstep<BATH and p <0.5) or p < epsilon:
                action = [random.randint(0, ppmaxlist[i]) for i in range(dnn_sum)]
            else:
                state_tensor = torch.tensor(state[np.newaxis, :]).to(device)
                action = network.select_action(state_tensor)   #list  (dnn_sum,)
            next_point_to_probe = action  # 动作选择
            ...
            if memory.size() > BEGIN_LEARN_SIZE:
                ...
                batch_state = torch.as_tensor(batch_state, dtype=torch.float).to(device) #(batch,n_states)
                batch_next_state = torch.as_tensor(batch_next_state, dtype=torch.float).to(device)#(batch,n_states)
                batch_action = torch.as_tensor(batch_action, dtype=torch.long).view(-1,dnn_sum).to(device)#(batch,dnn_sum)
                batch_reward = torch.as_tensor(batch_reward, dtype=torch.float).view(-1,1).to(device)#(batch,1)
                batch_done = torch.as_tensor(batch_done, dtype=torch.long).view(-1,1).to(device) #(batch,1)
                
                with torch.no_grad():
                    target_Q_next = target_network(batch_next_state)  #(batch,n_actions)
                    Q_next = network(batch_next_state)#(batch,n_actions)

                    left=0
                    right=ppmaxlist[0]+1
                    Q_max_action= torch.argmax(Q_next[:,left:right], dim=1, keepdim=True)  #(batch,1)   叠加为 (batch,dnn_sum)
                    for i in range(1,dnn_sum):
                        left=right
                        right+=ppmaxlist[i]+1
                        Q_max_actioni = torch.argmax(Q_next[:,left:right], dim=1, keepdim=True) 
                        Q_max_action=torch.cat((Q_max_action,Q_max_actioni),dim=1)
                       
                    y = batch_reward + GAMMA*torch.mul(target_Q_next.gather(1, Q_max_action),(1-batch_done))#(batch,dnn_sum)
            ...
            with open(ppof,"a") as f:
                ...
                f.write(f'     {str(sugendtime-sugstarttime):<30}{str(upstarttime-sugendtime):<30}{str(upendtime-upstarttime):<30}{str(loss)}')
                f.write("\n")
...
while True:
    try:
        ...
        if args.bandrand:
            ...
            res_ch=os.system("echo 6 | sudo -S wondershaper clear wlx08beac2cb0aa ")
            timecounter+=1 
    except KeyboardInterrupt or TypeError or OSError or ConnectionResetError:
        res_ch=os.system("echo 6 | sudo -S wondershaper clear wlx08beac2cb0aa ")
        print("poweraver",poweraverofthedivice)
        break
