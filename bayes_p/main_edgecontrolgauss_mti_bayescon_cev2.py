...
def edge(dnn,netnumer):
    ...
    #model.cuda()
    model.to(device)
    # model.to(dtype=torch.float32).to(device)
...
def communicationthwithclient(host,port,netnumber,actotal,mode):
    ...
    while True:
        try:
            ...
            # lastlatency=actuallatencylist[netnumber]
            # lastpp=partition_pointlistofthelatency[netnumber]
            # lock.release()
            # #判断是否延时过长，若是，本地执行。
            # if lastpp==ddpartition and lastlatency >= 10:
            #     ddpartition=actotal
            #client    
            ddpartition=actotal
            if mode =="client":
...
def guassthread(dnnnumber,username,dnn):
    ...
    pprf="latency/Gausslatency"+str(dnn)+str(dnnnumber)+".txt"
    allpf="latency/allpGauss"+str(dnn)+str(dnnnumber)+".txt"
    with open(pprf,'w') as prf:
        ...
        prf.write("\n")
    with open(allpf,'w') as allf:
        allf.write(f'step      pp       predict_client       predict_trans      predict_edge    ')  
        allf.write("\n")
    ...
    for epoch in range(30000):
        ...
        if readyflag :
            ...
            with open(allpf,"a") as af:
                if step%10==0:
                    for pi in range(len(mu[0])):
                        af.write(f"{str(step):<8}{str(pi):<8}{str(mu[0][pi][0]):<25}{str(mu[1][pi][0]):<25}{str(mu[2][pi][0]):<25}{str(mu_sum[pi]):<25}")
                        af.write(f'\n')    
        if step>0:
...
def edgetotalthread(port,dnn,totalnumber,username,mode):  #username=device , 
    global partitioncountdict
    #启动延时预测模型线程
    ...
    #启动与client通信线程
    th=threading.Thread(target=communicationthwithclient,args=(HOST,port,totalnumber,partitioncountdict[dnn],mode))    
    th.setDaemon(True)
    th.start()
...
def black_box_function(next_point_to_probe,numberlist,powermodel,username):#该设备上任务根据分割点做出决策，等待运行一段时间后,返回能耗
    ...
    try:
        ...
        for netnumber in numberlist:
            ...
            result.append(1)
            powerlist.append(-1)
            return result,powerlist,cpucountlist

    lockc.release()
    # result = [-1.0* power for power in powerlist]
    #result = [30.0 / power - 2 for power in powerlist]
    #缩放，0~1
...
def overallcontrol(username:str,mode): #每个设备上有一个 username，设备名称
    ...
    #加载功耗模型
    powermodelpath="models/powermodel_"+username+".pkl"
    ...
    while True:
        ...
        #记录
        bayesf="latency/bayeslog"+username+str(taskchangecount[username])+".txt"
        ...
        next_point_to_probe={}    # 例如：{'task0':4,'task3':7}
        #****循环部分***#
        while taskchangeflag[username]==False:
            cstep+=1
            for number in numberlist:
                if mode=='edge':
                    ...
                    next_point_to_probe[number]=pbounds_d[number][1]
            
            #2.等待一段时间，获取设备能耗。
            powerresult,powerlist,cpucountlist = black_box_function(next_point_to_probe,numberlist,powermodel,username)
...
def parse_argse():
    ...
    parser.add_argument("--mode",dest="mode",help="client or edge",default='client',type=str)
    args=parser.parse_args()
    return args

#与设备总控连接，共设备个数个线程，负责启动任务（接收到任务类型和端口号后）
def commuicatethread(device,port,lockcom,mode):
    global totalnumber
    global devicednnnumber
    global taskchangeflag
    global taskchangecount
    ...
    oct=threading.Thread(target=overallcontrol,args=(device,mode))
    oct.daemon=True
    oct.start()
    while True:
        ...
        #若收到请求信息，启动线程 
        edgethread=threading.Thread(target=edgetotalthread,args=(dnnport,dnn,totalnumber,device,mode))   #port,dnn,totalnumber,username
        edgethread.start()
        totalnumber+=1
        lockcom.release()
...
for i in range(len(devicelist)):
    ...
    thread=threading.Thread(target=commuicatethread,args=(devicelist[i],deviceportlist[i],lockcom,mode))
    thread.daemon=True
    thread.start()
 
timecounter=0
# bandwidthlist=[20, 15, 5, 15, 7, 10, 10, 20, 30, 7, 20, 15, 7, 5, 5, 30, 7, 15, 10, 7, 7, 7, 5, 25, 20, 7, 15, 30, 10, 20, 25, 5]
# bandwidthlist=[55, 55, 55, 55, 55, 5, 5, 5, 5, 5, 2, 2, 2, 2, 2, 55, 55, 55, 55, 55, 5, 5, 5, 5, 5, 2, 2, 2, 2, 2, 55, 55,55,55,55,5,5,5,5,5,2]
# bandwidthlist=[55, 55, 55, 55, 55, 10, 10, 10, 10, 10, 5, 5, 5, 5, 5, 55, 55, 55, 55, 55, 10, 10, 10, 10, 10, 5, 5, 5, 5, 5, 55, 55,55,55,55,10, 10, 10, 10, 10,5]
# bandwidthlist=[55, 55, 55,  15, 15, 15,  8, 8, 8,  55, 55, 55, 15, 15, 15,  8, 8, 8, 55,55,55, 15, 15, 15, 8]
# bandwidthlist=[55, 55, 55, 55, 55, 15, 15, 15, 15, 15, 8, 8, 8, 8, 8, 55, 55, 55, 55, 55, 15, 15, 15, 15, 15, 8, 8, 8, 8, 8, 55, 55,55,55,55,15, 15, 15, 15, 15, 8]
# bandwidthlist=[55, 55, 55, 15, 15, 15, 8, 8, 8, 15, 15, 15, 8, 8, 8, 55, 55, 55, 15, 15, 15, 8,8,8]
bandwidthlist=[55,55,8,8,15,15,5,5,8,8,15,15,55,55,15,15,5,5,8,8,8,8,5,5,55,55,55,55]  #多写了一个。
