...
class Net(nn.Module):
    ...
    def __init__(self, n_states, n_hidden, n_actions):
        ...
        #self.bn2=nn.BatchNorm1d(n_actions)
...
class DQN:
    ...
    def take_action(self, state):
        ...
        #print("state before take action",state)
    ...
    def update(self, transition_dict):  # 传入经验池中的batch个样本
        ...
        #print("states in update",transition_dict['states'])
        #print("actions in update",transition_dict['actions'])
        states = torch.tensor([transition_dict['states']], dtype=torch.float).view(-1,1)
        # 获取当前时刻采取的动作 tuple_shape=[b]，维度扩充 [b,1]
        actions = torch.tensor(transition_dict['actions']).view(-1,1)
        # 当前状态下采取动作后得到的奖励 tuple=[b]，维度扩充 [b,1]
        rewards = torch.tensor(transition_dict['rewards'], dtype=torch.float).view(-1,1)
        # 下一时刻的状态 array_shape=[b,4]
        next_states = torch.tensor([transition_dict['next_states']], dtype=torch.float).view(-1,1)
