...
def clientinfer(dnn_model,host,port,breaktime,netnumber):
    ...
    if dnn_model == 'vgg':
        ...
        weights_path=str(parent_dir)+'/models/mobileformer.pth.tar'
    ...
    filepath=str(this_dir)+"/latency/latency_actula"+str(netnumber)+".txt"
...
while True:
    try:
        ...
        if timenow-timestart>=(timecounter+1)*60:
            ...
            ##带宽控制
            if args.bandrand:
                bandwidthlist=[50,5,10,10,50,5,10,10,50,5,5,50,10,10]
